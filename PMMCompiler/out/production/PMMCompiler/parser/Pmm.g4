...
main returns [Definition ast] locals [List<VariableDefinition> varDefinitions = new ArrayList<VariableDefinition>(),
                                        List<Statement> statements = new ArrayList<Statement>()]:
            T='def' 'main' '(' ')'':''{' (vd=variableDefinition';' { $varDefinitions.addAll($vd.ast); })* (st1=statement { $statements.addAll($st1.ast); })* '}'
                {
                    $ast = new FunctionDefinition($T.getLine(), $T.getCharPositionInLine() + 1,
                        new FunctionType($T.getLine(), $T.getCharPositionInLine() + 1,
                            new ArrayList<VariableDefinition>(), VoidType.getInstance()), "main", $varDefinitions, $statements);
                }
            ;

statement returns [List<Statement> ast = new ArrayList<Statement>()]
    locals [List<Statement> elseBody = new ArrayList<Statement>()]:
            T='print' ex=expressions';'
                {
                    for (Expression expression : $ex.ast) {
                        $ast.add(new Print($T.getLine(), expression.getColumn(), expression));
                    }
                }
            | T='input' ex=expressions';'
                {
                    for (Expression expression : $ex.ast) {
                        $ast.add(new Input($T.getLine(), expression.getColumn(), expression));
                    }
                }
...
type returns [Type ast]:
            ...
            | T='struct''{'structFieldList=structFields'}'
                {
                    $ast = new StructType($T.getLine(), $T.getCharPositionInLine() + 1, $structFieldList.ast);
                }
            ;

structFields returns [List<StructField> ast = new ArrayList<StructField>()]:
            (idList=ids ':' tp=type ';'
                {
                    for (Variable v : $idList.ast) {
                        StructField a = new StructField(v.getLine(), v.getColumn(), $tp.ast, v.getName());
                        if ($ast.contains(a)) {
...
functionDefinition returns [FunctionDefinition ast]
    locals [Type returnType = VoidType.getInstance(),
            List<VariableDefinition> paramsList = new ArrayList<VariableDefinition>(),
            List<VariableDefinition> varDefinitions = new ArrayList<VariableDefinition>(),
            List<Statement> statements = new ArrayList<Statement>()]:
            T='def' ID '(' (p=params { $paramsList = $p.ast; })? ')'':' (tp=type { $returnType = $tp.ast; })? '{'
                (vd=variableDefinition';' { $varDefinitions.addAll($vd.ast); })* (st1=statement { $statements.addAll($st1.ast); })* '}'
                {
                    $ast = new FunctionDefinition($T.getLine(), $T.getCharPositionInLine() + 1,
                        new FunctionType($returnType.getLine(), $returnType.getColumn(), $paramsList, $returnType), $ID.text, $varDefinitions, $statements);
                }
            ;
